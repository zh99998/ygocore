...
int32 card::filter_summon_procedure(uint8 playerid, effect_set* peset, uint8 ignore_count) {
	...
	if(min == 0)
		return TRUE;
	card_set cset;
	uint32 m = pduel->game_field->get_summon_release_list(this, &cset);
	if(m >= min)
		return TRUE;
	return FALSE;
}
...
int32 card::is_can_be_summoned(uint8 playerid, uint8 ignore_count, effect* peffect) {
	...
	if(current.location == LOCATION_MZONE) {
		...
		if((peffect && res < 0) || (!peffect && (!res or res == -2) && !proc.count)) {
			pduel->game_field->restore_lp_cost();
			return FALSE;
		}
	}
...
int32 card::is_setable_mzone(uint8 playerid, uint8 ignore_count, effect* peffect) {
	...
	if((peffect && res < 0) || (!peffect && (!res or res == -2) && !eset.count) ) {
		pduel->game_field->restore_lp_cost();
		return FALSE;
	}
	pduel->game_field->restore_lp_cost();
